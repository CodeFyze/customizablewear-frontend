			...
			<div className=' w-full h-auto md:w-2/3 flex justify-center shadow-lg'>
				{selectedShirt && (
					<img
						...
						className={`w-full  object-contain h-96 md:m-10 max-w-xs md:max-w-3xl p-4 md:p-0 transform transition-transform duration-200 ${isAnimating ? 'scale-10 opacity-50' : 'scale-10 opacity-100'
							}`}
					/>
				)}
			</div>
			...
			<div className='h-auto px-2 w-full md:w-1/3 md:pr-10 '>
					...
					{products.map((product, index) => (
						<div key={index} className='text-center'>
							<img
								...
								className={`w-24 h-24 object-cover rounded-md cursor-pointer border border-gray-300 hover:border-orange-500 ${selectedProduct === product ? 'border-orange-500' : ''
									}`}
				...
				<div className='flex space-x-2 mt-4'>
					{selectedProduct?.colors?.map((color, index) => (
						<div
							key={index}
							className={`w-8 h-8 rounded-full border border-gray-300 cursor-pointer ${selectedColor === color ? 'border-orange-500 outline outline-2' : 'hover:border-orange-500'
								}`}
